...
import { TrendingUp, Bot, Loader, AlertTriangle, Shield, Download, Layers } from 'lucide-react'
import { getAnalyticsData, callAI, getProgrammes, getComparisonData } from '../lib/supabase'
...
const COLORS = { MW:'#dc2626', SD:'#d97706', CD:'#2563eb', 'CD-D':'#7c3aed' }
...
export default function Analytics() {
  ...
  useEffect(() => {
    ...
    getProgrammes().then(d => setPrograms(d||[]))
  }, [programmeId])
  ...
  const runAI = async () => {
    ...
    try {
      const stats = {
        ...
        findings: { total:data.findings.length },
        deficiencies: { mw_open:mwCount, sd_open:sdCount },
        ...
        ipe: { total:ipeTotal, validated:ipeValidated },
  ...
  const doExportPDF = () => {
    exportPDF({
      filename: `${programme?.name||'SOX'}_Analytics`.replace(/\s+/g,'_'),
      title: `SOX Audit Analytics — ${programme?.name||''}`,
      subtitle: `FY${programme?.fiscal_year||''} · ${new Date().toLocaleDateString()}`,
      sections: [
        { heading:'Key Indicators', columns:['Metric','Value'], rows:[
          ['Controls untested',`${untestedPct}%`],['Open MW',mwCount],['Open SD',sdCount],
          ['Exception rate',`${exceptionRate}%`],['IPE validated',`${ipeValidated}/${ipeTotal}`],
        ]},
        ...
        ...(ai?[{heading:'AI Opinion',text:`${ai.overall_opinion}: ${ai.executive_summary}`}]:[]),
      ]
    })
  }
  ...
  const runComparison = async () => {
    setCompLoading(true)
    const ids = [...new Set([programmeId, ...selectedIds])]
    getComparisonData(ids).then(setCompData).finally(()=>setCompLoading(false))
  }

  return (
    <div className="p-6 max-w-6xl mx-auto">
      <PageHeader eyebrow={<><TrendingUp size={12}/>Analytics</>} title="Audit analytics"
        subtitle={`${programme?.name} · FY${programme?.fiscal_year}`}
        actions={<button className="btn btn-outline btn-sm" onClick={doExportPDF}><Download size={13}/>Export PDF</button>} />

      <div className="flex gap-2 mb-5">
        <button onClick={()=>setTab('current')} className={`btn btn-sm ${tab==='current'?'btn-primary':'btn-outline'}`}><TrendingUp size={13}/>Current engagement</button>
        <button onClick={()=>setTab('compare')} className={`btn btn-sm ${tab==='compare'?'btn-primary':'btn-outline'}`}><Layers size={13}/>Compare engagements</button>
      </div>
        ...
        <div>
          ...
          <div className="grid grid-cols-1 md:grid-cols-2 gap-4 mb-6">
            <div className="card">
              ...
              <ResponsiveContainer width="100%" height={200}>
                <BarChart data={rcmByDomain} barSize={16}>
                  <XAxis dataKey="domain" tick={{fontSize:11}}/><YAxis tick={{fontSize:11}} allowDecimals={false}/>
            ...
            <div className="card">
              ...
              {defByClass.length>0?(
                <ResponsiveContainer width="100%" height={200}>
                  <PieChart><Pie data={defByClass} dataKey="value" nameKey="name" cx="50%" cy="50%" outerRadius={70} label={({name,value})=>`${name}: ${value}`} labelLine={false}>
                    {defByClass.map(e=><Cell key={e.name} fill={COLORS[e.name]||'#94a3b8'}/>)}
                ...
                <ResponsiveContainer width="100%" height={200}>
                  <BarChart data={remByStatus} layout="vertical" barSize={14}>
                    ...
                    <Bar dataKey="value" radius={[0,4,4,0]}>
                      {remByStatus.map(e=><Cell key={e.name} fill={e.name==='Closed'?'#16a34a':e.name==='Implemented'?'#2563eb':'#d97706'}/>)}
                    </Bar>
                  </BarChart>
                </ResponsiveContainer>
                ...
                <ResponsiveContainer width="100%" height={200}>
                  <BarChart data={vendorReliance} barSize={20}>
                    ...
                    <Bar dataKey="value" radius={[4,4,0,0]}>
                      {vendorReliance.map(e=><Cell key={e.name} fill={e.name==='place'?'#16a34a':e.name==='partial'?'#d97706':'#94a3b8'}/>)}
                    </Bar>
                  </BarChart>
                </ResponsiveContainer>
          ...
          <div className="card">
            ...
            {ai?(
              <div className="space-y-4">
                ...
                <div><div className="text-xs font-bold uppercase tracking-wide text-gray-500 mb-2">Executive summary</div><p className="text-sm text-gray-700 dark:text-gray-300 leading-relaxed">{ai.executive_summary}</p></div>
                {ai.key_risks?.length>0&&(
                  <div><div className="text-xs font-bold uppercase tracking-wide text-gray-500 mb-2">Key risks</div>
                  <div className="space-y-2">{ai.key_risks.map((r,i)=>(
                ...
                {ai.recommendations?.length>0&&(
                  <div><div className="text-xs font-bold uppercase tracking-wide text-gray-500 mb-2">Recommendations</div>
                  <div className="space-y-2">{ai.recommendations.map((r,i)=>(
              ...
              <div className="text-center py-8 text-gray-400">
                ...
                <p className="text-sm">Click "Generate narrative" to produce an AI audit summary.</p>
              </div>
            )}
          </div>
        </div>
      ...
      {tab==='compare' && (
        <div className="space-y-5">
          <div className="card">
            <h3 className="font-semibold text-sm mb-3 text-gray-900 dark:text-white">Select engagements to compare</h3>
            <div className="space-y-2 mb-4 max-h-48 overflow-y-auto">
              {programmes.filter(p=>p.id!==programmeId).length===0
                ? <p className="text-sm text-gray-400">No other engagements found. Create another engagement to compare.</p>
                : programmes.filter(p=>p.id!==programmeId).map(p=>(
                  <label key={p.id} className="flex items-center gap-2 text-sm cursor-pointer p-2 rounded-lg hover:bg-gray-50 dark:hover:bg-dark-surface-3">
                    <input type="checkbox" checked={selectedIds.includes(p.id)}
                      onChange={e=>setSelectedIds(ids=>e.target.checked?[...ids,p.id]:ids.filter(i=>i!==p.id))}/>
                    <span className="font-medium">{p.name}</span>
                    <span className="badge badge-gray">{p.fiscal_year}</span>
                  </label>
                ))
              }
            </div>
            <button className="btn btn-primary btn-sm" onClick={runComparison} disabled={compLoading||selectedIds.length===0}>
              {compLoading?<><Loader size={13} className="animate-spin"/>Loading…</>:'Compare'}
            </button>
          </div>
            ...
            <>
              <div className="card overflow-x-auto p-0">
                <table className="data-table">
                  <thead><tr>
                    <th>Engagement</th><th>FY</th><th>Controls</th><th>Tested %</th>
                    <th>Findings</th><th>Open MW</th><th>Open SD</th><th>Rem. closed</th>
                  </tr></thead>
                  <tbody>{compData.map(d=>(
                    <tr key={d.id} className={d.id===programmeId?'bg-blue-50 dark:bg-blue-900/10':''}>
                      <td className="font-medium">{d.name}{d.id===programmeId&&<span className="badge badge-blue ml-2 text-xs">Current</span>}</td>
                      <td>{d.fiscal_year}</td>
                      <td>{d.controls_total}</td>
                      <td><span className={`badge ${d.controls_total>0&&(d.controls_tested/d.controls_total)>0.8?'badge-green':'badge-amber'}`}>{d.controls_total>0?Math.round((d.controls_tested/d.controls_total)*100):0}%</span></td>
                      <td>{d.findings_total}</td>
                      <td><span className={`badge ${d.mw_open>0?'badge-red':'badge-green'}`}>{d.mw_open}</span></td>
                      <td><span className={`badge ${d.sd_open>0?'badge-amber':'badge-green'}`}>{d.sd_open}</span></td>
                      <td>{d.rem_closed}</td>
                    </tr>
                  ))}</tbody>
                </table>
              ...
              <div className="card">
                <h3 className="font-semibold text-sm mb-4 text-gray-900 dark:text-white">Findings comparison</h3>
                <ResponsiveContainer width="100%" height={220}>
                  <BarChart data={compData.map(d=>({name:d.fiscal_year||d.name.slice(0,12),'Major NC':d.major_nc,'Minor NC':d.minor_nc,MW:d.mw_open,SD:d.sd_open}))} barSize={14}>
                    <XAxis dataKey="name" tick={{fontSize:11}}/><YAxis tick={{fontSize:11}} allowDecimals={false}/>
                    <Tooltip contentStyle={{fontSize:11}}/><Legend wrapperStyle={{fontSize:11}}/>
                    <Bar dataKey="MW" fill="#dc2626"/>
                    <Bar dataKey="SD" fill="#d97706"/>
                    <Bar dataKey="Major NC" fill="#7c3aed"/>
                    <Bar dataKey="Minor NC" fill="#2563eb"/>
                  </BarChart>
                </ResponsiveContainer>
              </div>
            </>
